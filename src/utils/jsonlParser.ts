    ...
    if (line.trim()) {
      try {
        ...
        const destination: Destination = {
          place_id:
            rawDestination.restaurant_id?.toString() ||
            rawDestination.place_id ||
            '',
          order: rawDestination.restaurant_id || rawDestination.order || 0,
          nama:
            rawDestination.nama_destinasi ||
            rawDestination.nama ||
            'Nama tidak tersedia',
          ...
          deskripsi:
            rawDestination.deskripsi ||
            generateDefaultDescription(
              rawDestination.nama_destinasi || rawDestination.nama,
              rawDestination.kategori
            ),
          rating: rawDestination.rating || generateDefaultRating(),
          alamat:
            rawDestination.alamat ||
            'Alamat belum tersedia, Surabaya, Jawa Timur',
          jam_buka: rawDestination.jam_buka || '08:00 - 22:00 WIB',
          image_url:
            rawDestination.image_url ||
...
function generateDefaultDescription(
  ...
  return (
    descriptions[mainKategori] ||
    `${nama} adalah destinasi menarik yang patut dikunjungi saat berada di Surabaya. Menawarkan pengalaman yang berkesan untuk wisatawan.`
  );
}
